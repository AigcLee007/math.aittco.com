...
export function AppChat() {
  ...
  const handleImagineFromText = React.useCallback(async (conversationId: DConversationId, subjectText: string) => {
    ...
    if (!cHandler.isValid()) return;
    const userImagineMessage = createDMessagePlaceholderIncomplete('user', `姝ｅ湪鎬濊€冧富棰?..`); // [chat] append user:imagine prompt
    ...
    await imaginePromptFromTextOrThrow(subjectText, conversationId)
      ...
      .catch((error: any) => {
        ...
        cHandler.messageFragmentReplace(userImagineMessage.id, userImagineMessage.fragments[0].fId, createErrorContentFragment(`璇锋眰鍥惧儚鎻愮ず璇嶆椂鍑洪敊銆?${error?.message || ''}`), true);
      });
  }, [handleExecuteAndOutcome]);
  ...
  useGlobalShortcuts('AppChat', React.useMemo(() => [
    ...
    { key: 'b', ctrl: true, shift: true, disabled: isFocusedChatEmpty, action: handleMessageBeamLastInFocusedPane, description: '铻嶅悎缂栬緫' },
