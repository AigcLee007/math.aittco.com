...
export function anthropicAccess(access: AnthropicAccessSchema, apiPath: string, options?: AnthropicHeaderOptions): { headers: HeadersInit, url: string } {
  ...
  // Enforce relay/custom host: no official default host fallback.
  if (!anthropicKey)
  ...
  // API host
  const rawHost = access.anthropicHost
    || env.ANTHROPIC_API_HOST
    || env.AITTCO_API_HOST
    || env.OPENAI_API_HOST
    || '';
  ...
  if (heliKey) {
    if (!llmsHostnameMatches(anthropicHost, DEFAULT_HELICONE_ANTHROPIC_HOST))
  ...
  return {
    headers: {
      ...
      // Relay support: bearer token for relay-compatible gateways.
      ...(!heliKey ? { 'Authorization': `Bearer ${anthropicKey}` } : {}),
