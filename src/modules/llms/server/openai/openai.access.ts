...
export function llmsFixupHost(host: string, apiPath: string): string {
  ...
  try {
    ...
    if ((normalizedPath === '/v1' || normalizedPath === '/v1beta' || normalizedPath === '/v1alpha')
      && (apiPath.startsWith('/v1/') || apiPath.startsWith('/v1beta/') || apiPath.startsWith('/v1alpha/')))
      url.pathname = '';
    host = url.toString();
  } catch {
    // Keep original host when normalization fails.
  }
...
function llmsNormalizedHostOriginAndPath(hostUrl: string | undefined): string {
  if (!hostUrl)
    return '';
  try {
    const url = new URL(hostUrl.startsWith('http') ? hostUrl : `https://${hostUrl}`);
...
export function openAIAccess(access: OpenAIAccessSchema, modelRefId: string | null, apiPath: string): { headers: HeadersInit, url: string } {
  switch (access.dialect) {
    ...
    case 'openai': {
      ...
      const trustedRelayHosts = [
        env.OPENAI_API_HOST,
        env.AITTCO_API_HOST,
      ]
        .map(llmsNormalizedHostOriginAndPath)
        .filter(Boolean);
      const requestedHost = llmsNormalizedHostOriginAndPath(access.oaiHost);
      const isTrustedServerRelayHost = !!requestedHost && trustedRelayHosts.includes(requestedHost);
      if (access.oaiHost) {
        // Client controls the endpoint. If it's a server-trusted relay host, allow server-side fallback credentials.
        oaiKey = access.oaiKey || (isTrustedServerRelayHost ? (env.OPENAI_API_KEY || env.AITTCO_API_KEY || '') : '');
        oaiHost = access.oaiHost;
        oaiOrg = access.oaiOrg || (isTrustedServerRelayHost ? (env.OPENAI_API_ORG_ID || '') : '');
        heliKey = access.heliKey || (isTrustedServerRelayHost ? (env.HELICONE_API_KEY || false) : false);
      } else {
        // Client hasn't touched the endpoint: server infrastructure
        oaiKey = access.oaiKey || env.OPENAI_API_KEY || env.AITTCO_API_KEY || '';
