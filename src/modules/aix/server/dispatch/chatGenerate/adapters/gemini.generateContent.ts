...
export function aixToGeminiGenerateContent(model: AixAPI_Model, _chatGenerate: AixAPIChatGenerate_Request, geminiSafetyThreshold: GeminiWire_Safety.HarmBlockThreshold, jsonOutput: boolean, _streaming: boolean): TRequest {
  ...
  // [Gemini, 2026-02-08] Verbosity control - official OpenAI parameter (low/medium/high, default: medium)
  // We map this to a system instruction, as Gemini does not have a native parameter for this
  if (model.vndOaiVerbosity && model.vndOaiVerbosity !== 'medium') {
    const verbosityInstruction = `Answer with ${model.vndOaiVerbosity} detail.`;
    if (!systemInstruction) {
      systemInstruction = { parts: [GeminiWire_ContentParts.TextPart(verbosityInstruction)] };
    } else {
      // append to the end of the first part if text, or add a new part
      const lastPart = systemInstruction.parts[systemInstruction.parts.length - 1];
      if (lastPart && 'text' in lastPart)
        lastPart.text += `\n${verbosityInstruction}`;
      else
        systemInstruction.parts.push(GeminiWire_ContentParts.TextPart(verbosityInstruction));
    }
  }
  ...
  // Important: internal thinking and visible thought summaries are not the same thing.
  // We only request visible thoughts when the user explicitly enabled them, otherwise
  // Gemini may surface reasoning summaries mid-stream and make the UI appear to
  // "roll back" from an already visible answer into a thinking state.
  if (model.vndGeminiShowThoughts === true || model.vndGeminiThinkingBudget !== undefined || model.vndGeminiThinkingLevel) {
    ...
    // Visible thought summaries must be opt-in.
    if (model.vndGeminiShowThoughts === true)
  ...
  // [Custom] Default to 'unfiltered' (auto search) if not set, to ensure relay models have search enabled
  const effectiveGoogleSearch = model.vndGeminiGoogleSearch ?? 'unfiltered';
  if (effectiveGoogleSearch && !skipHostedToolsDueToCustomTools) {
    if (!payload.tools) payload.tools = [];

    // Build the Google Search tool configuration
    const googleSearchTool: NonNullable<TRequest['tools']>[number] = {
      googleSearch: _buildGoogleSearchConfig(effectiveGoogleSearch),
    };
  ...
  // Debug: log the payload to see if tools are present
  console.log('[Gemini Dispatch] Final Payload tools:', JSON.stringify(payload.tools, null, 2));
...
function _toApproximateGeminiDocPart(aixPartsDocPart: AixParts_DocPart): GeminiWire_ContentParts.ContentPart {
  // NOTE: doc parts are text-only in the current AIX schema.
