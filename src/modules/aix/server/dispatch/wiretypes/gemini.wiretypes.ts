...
export namespace GeminiWire_API_Generate_Content {
  ...
  const CitationMetadata_schema = z.object({
    ...
    citationSources: z.array(CitationSource_schema).optional(),
    // Some relays/providers return this alternate key while keeping the same item shape.
    citations: z.array(CitationSource_schema).optional(),
  }).transform((metadata) => ({
    ...metadata,
  ...
  const Candidate_schema = z.object({
     ...
     */
    finishReason: z.union([FinishReason_enum, z.string()]).nullable().optional(),
    /**
  ...
  const UsageMetadata_schema = z.object({
    ...
    promptTokenCount: z.number().optional(), // [Gemini, 2025-02-09] Made optional for Word doc parsing
    ...
    promptTokensDetails: z.array(ModalityTokenCount_schema).nullish(),
    cacheTokensDetails: z.array(ModalityTokenCount_schema).nullish(),
...
export namespace GeminiWire_API_Models_List {
  ...
  const Model_schema = z.object({
    ...
    displayName: z.string().nullable().optional(),    // Human readable
    description: z.string().nullable().optional(),
    inputTokenLimit: z.number().nullable().optional(),
    outputTokenLimit: z.number().nullable().optional(),
    supportedGenerationMethods: z.array(z.union([Methods_enum, z.string()])).nullable().optional(), // relaxed with z.union to not break on expansion
